...
void createCharts()              //!!! Use this to create ALL the charts we need!!!               By chuan:)
{
  ...
  ArrayList<String> arrDelayList = new ArrayList<String>();
  ...
  for (Map.Entry<Integer, Integer> entry : arrDelayFreq.entrySet()) 
  {
       ...
       arrDelayList.add(""+entry.getKey());
  }
  String[] delayLabels = new String[numOfFlightsByArrDelay.size()];
  ...
  for (int i = 0; i < numOfFlightsByArrDelay.size(); i++) 
  {
       ...
       delayLabels[i]=arrDelayList.get(i);
      // System.out.println(delayLabels[i]+" : "+arrDelayFreqArray[i]);
  }
